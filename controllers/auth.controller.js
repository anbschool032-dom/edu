...
//   try {
//     // 1. រក Session តាមរយៈ Verify Token
...
//     // 3. Update User Status ទៅ Active
//     await User.update(
//       { status: 'active', email_verified_at: new Date() },
//       { where: { id: user.id } }
//     );

//     // 4. 🔥 Generate New Tokens (សម្រាប់ Auto Login)
//     const { accessToken, refreshToken } = generateTokens(user);

//     // 5. Update Session ជាមួយ Token ថ្មី (ជំនួសឱ្យការលុបចោល)
//     // ដើម្បីឱ្យ Cookie ដំណើរការ និងអាច Refresh Token បាន
...
//     // 7. ផ្ញើ Access Token និង Role ទៅ Frontend
//     res.json({ 
...
const login = async (req, res) => {
  ...
  try {
    // 1. Find User AND Include Profile Data
    const user = await User.findOne({ 
      where: { email },
      include: [
        { model: Admin, as: 'admin', required: false },
    ...
    // 2. User Not Found Check (Specific Message for Debugging)
    if (!user) {
        // 🔥 Change to "User not found" so Frontend Alert shows it
        return res.status(404).json({ message: 'User not found' }); 
    }
    ...
    const isMatch = await bcrypt.compare(password, user.password);
    if (!isMatch) {
        // 🔥 Change to "Incorrect password" so Frontend Alert shows it
...
const resetPassword = async (req, res) => {
  ...
  try {
    ...
    if (new Date() > new Date(resetRecord.expires_at)) return res.status(400).json({ message: 'Token has expired' });
...
module.exports = { 
  registerMentor, login, refreshToken, logout, getMe, verifyEmail, updatePassword, forgotPassword, resetPassword
};